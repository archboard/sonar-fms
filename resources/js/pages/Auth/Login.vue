    ...
    <form @submit.prevent="submit" data-cy="form">
      <div v-if="tenant.allow_oidc_login">
        <span class="rounded-md shadow-sm self-start flex w-full">
          <a :href="$route('oidc.login')" class="px-4 py-3 text-base leading-6 flex w-full border border-transparent font-medium rounded-md text-white focus:outline-none transition ease-in-out duration-150 items-center justify-center text-white" style="background-color: #00427c">

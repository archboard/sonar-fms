<template>
  <Authenticated>
    <template v-slot:actions>
      <Button component="inertia-link" href="/layouts/receipts/create" size="sm">
        {{ __('Add') }}
      </Button>
    </template>
      ...
      <Tbody>
        ...
        >
          ...
          <Td class="text-right space-x-3">
            <VerticalDotMenu>
              <div class="p-1">
                <SonarMenuItem v-if="!layout.is_default && can('layouts.update')" :href="`/layouts/receipts/${layout.id}/default`" method="post" as="button" is="InertiaLink">
                  {{ __('Make default') }}
                </SonarMenuItem>
                <SonarMenuItem :href="`/layouts/receipts/${layout.id}/preview`" is="a">
                  {{ __('Preview') }}
                </SonarMenuItem>
                <SonarMenuItem v-if="can('layouts.update')" :href="`/layouts/receipts/${layout.id}/edit`" is="InertiaLink">
                  {{ __('Edit') }}
                </SonarMenuItem>
                <SonarMenuItem v-if="can('layouts.delete')" :href="`/layouts/receipts/${layout.id}`" is="button" @click.prevent="layoutToDelete = layout">
                  {{ __('Delete') }}
                </SonarMenuItem>
              </div>
            </VerticalDotMenu>
        ...
        <tr v-if="layouts.meta.total === 0">
          <Td class="text-center" colspan="3">
            {{ __('No layouts exist.') }} <Link href="/layouts/receipts/create">{{ __('Add one') }}</Link>.
          </Td>
        </tr>
      </Tbody>
    </Table>
...
export default defineComponent({
  ...
  setup () {
    ...
    const deleteLayout = () => {
      Inertia.delete(`/layouts/receipts/${layoutToDelete.id}`, {
        preserveScroll: true
      })
    }
